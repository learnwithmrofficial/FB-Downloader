        ...
        <div className="p-6 rounded-3xl glass-panel border border-white/10 space-y-6 shadow-2xl animate-fade-in">
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 space-y-4">
              ...
              {/* All Available Formats Table */}
              {mediaInfo.downloadOptions && mediaInfo.downloadOptions.length > 0 && (
                <div className="mt-6 pt-5 border-t border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-gray-200 uppercase tracking-wider flex items-center gap-2">
                      <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                      All Detected Stream Options
                    </h4>
                    <span className="text-[11px] text-emerald-400 font-mono flex items-center gap-1">
                      <Clock className="w-3 h-3" /> Full Length: {mediaInfo.durationFormatted}
                    </span>
                  </div>

                  <div className="rounded-2xl bg-black/40 border border-white/10 overflow-hidden divide-y divide-white/5">
                    {mediaInfo.downloadOptions.map((opt, i) => (
                      <div key={i} className="p-3 flex items-center justify-between gap-3 hover:bg-white/[0.03] transition-colors">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs ${opt.format === 'MP3' || opt.format === 'M4A' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>
                            {opt.format}
                          </div>
                          <div>
                            <div className="text-xs font-semibold text-white flex items-center gap-2">
                              ...
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-gray-300 font-mono">
                                ~{opt.sizeMB > 1024 ? `${(opt.sizeMB / 1024).toFixed(2)} GB` : `${opt.sizeMB} MB`}
                              </span>
                            </div>
                            <div className="text-[10px] text-gray-400">
                              {opt.format === 'MP3' || opt.format === 'M4A' ? 'Audio Stream' : 'Video + Audio Stream'} • Full {mediaInfo.durationFormatted} Duration
                            </div>
                          </div>
                        </div>
                        ...
                        <a
                          ...
                          download={`${mediaInfo.title}.${opt.format.toLowerCase()}`}
                          className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-lg shadow-emerald-600/20 flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
                        >
